/*
	...
	思路：Tarjan在找到割点的时候判断一下是不是环，是环的话点数够不够
		  跟“圆桌会议”很像的算法
		  实现还是挺好看的：用fuck[]记录路径，跟以往把当前选择的点都记录到路径数组里的实现不一样，我这里是dfs递归，然后dfs返回来true才用路径数组记录当前点。虽说是小事情，但感觉还是比较简单巧妙~
...
bool dye(int u, int pre, int sum, int c)		//染色法判断是不是环，染色其实只是为了判点重...	sum是点数
{
	if(color[u] != 0) {
		if(sum > K + 1 && u == st) {			//是否数目>=K+1 && 是不是以u点开头的环
												//判断u == st 很重要啊，因为我们的计数sum是从st开始的，所以要判断u == st
												//因为我们对每个割点都有dfs一次判环，所以不用怕漏掉
			fuck[sum] = u;
			fuck[0] = sum;
			return true;
		}

/*
    ...
    思路：第一反应是状压DP，但感觉状态之间的关系很复杂，搞不动。
          但是只要能证明一个结论，这道题就很简单了。要证：存在一种最终布局，使得最优移动过程中任何有皇后不互挡，也就是说我们可以不用考虑“不能跨皇后”这个限制，直接“穿越”做。
          我画了个4*4的图，然后幸运地得出了证明：假设现在在i点的皇后要到k点去（那k点当然是最终布局的皇后点啦，不然到那里去干嘛），中间有个j（j点有个皇后）隔住了。 那么k与j就在一条直线上，也就是说 j 不是最终皇后点，那么 j 就应该要先挪到她相应的点去，等她挪完后就不挡住i 到达 k 点了。也就表明了，最优移动过程种任何皇后不互挡。

          证了上述结论就好办了，也就是说题意“不能跨皇后移动”这一条件完全是唬人的，这就是裸二分图带权匹配。
		  用二分图带权匹配KM算法或者最大流最小费用流MCMF都可以做，费用为移动步数。我是用KM 做的。
		  先预处理出8*8上所有合理布局，一共92种。然后对于每种合理布局，拿来跟初始布局搞二分图带权匹配，然后取结果的最小值即可。易知这个二分图是完全二分图（表述不太准确，就是X部Y部两两有边）。
...
bool check(int row, int col)//判断现已放皇后是否会相互攻击
{
    return l[0][col] && l[1][row+col] && l[2][row-col+N];
}
void init(int row)		//预处理出所有最终布局，共有92种
{
    if(row == N) {

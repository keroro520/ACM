...
int dis(int x1, int y1, int x2, int y2)
{
    ...
    if(x1 + y1 == x2 + y2 || (x1 - y1) == (x2 - y2)) return 1;      //妹的，本来写的是对的，看了neko的，然后改错了=_=
    return 2;
}
void build(int * map) 
{
...
int main()
{
    ...
    while(cases--) {
        ...
        for(int i = 0; i < Top; i++) {                  //枚举每一种最终布局
            build(map[i]);
            //if(KM() == 6 && ans != 6) {
            //    for(int j = 0; j < N; j++) printf("(%d,%d)  ", a[j][0]+1,a[j][1]+1);
            //    printf("\n");
            //    for(int j = 0; j < N; j++) printf("(%d,%d)  ", j+1, 1+map[i][j]);
            //    printf("\n");
            //}
            ans = min(ans, KM());
        }
